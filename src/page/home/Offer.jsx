import React from "react";

const Offer = () => {
  return (
    <section className=" py-12">
      <div className=" mx-auto ">
        <h2 className="text-3xl font-bold text-center mb-8">
          Stay Updated with Our Highlights
        </h2>

        {/* Highlights Section */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
          <div className=" bg-gray-900 shadow-lg rounded-lg p-6 text-center">
            <h3 className="text-xl  text-gray-300 font-semibold mb-4">Featured Product</h3>
         
            <p className="text-gray-300">
              Discover our latest and greatest products, carefully crafted to
              meet your needs.
            </p>
            <button className="mt-4 px-6 py-2 bg-blue-500 text-white rounded-md hover:bg-blue-600 transition-all duration-300">
              Learn More
            </button>
          </div>

          <div className="bg-gray-800 shadow-lg rounded-lg p-6 text-center">
            <h3 className="text-xl text-gray-300 font-semibold mb-4">Special Offer</h3>
            <p className="text-2xl font-bold text-red-500 mb-2">50% Off!</p>
            <p className="text-gray-300">
              Limited-time discounts on our best-selling items. Don’t miss out!
            </p>
            <button className="mt-4 px-6 py-2 bg-green-500 text-white rounded-md hover:bg-green-400 transition-all duration-300">
              Shop Now
            </button>
          </div>

          <div className="bg-gray-800 shadow-lg rounded-lg p-6 text-center">
            <h3 className="text-xl  text-gray-300 font-semibold mb-4">Join Our Newsletter</h3>
            <p className="text-gray-300 mb-4">
              Get exclusive updates, promotions, and insights straight to your
              inbox.
            </p>
            <input
              type="email"
              placeholder="Enter your email"
              className="border border-gray-300 rounded-lg px-4 py-2 w-full mb-4"
            />
            <button className="px-6 py-2 bg-indigo-500 text-white rounded-md hover:bg-indigo-400 transition-all duration-300">
              Subscribe
            </button>
          </div>
        </div>

        {/* Promotional Banner */}
        <div className=" text-white py-12 rounded-lg text-center">
          <h3 className="text-2xl text-gray-300 font-bold mb-4">Exclusive Promotion</h3>
          <p className="text-lg text-gray-300 mb-6">
            Unlock special deals and early access to our new collections by
            joining our community today.
          </p>
          <button className="px-5 py-2 border-2 border-sky-400 text-white font-semibold rounded-lg shadow-md transition-all duration-300 hover:bg-blue-700 hover:scale-105 hover:shadow-blue-500/50">
            Join Now
          </button>
        </div>
      </div>
    </section>
  );
};

export default Offer;
