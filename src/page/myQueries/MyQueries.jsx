...
const MyQueries = () => {
    ...
    const handleDelete = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    await axiosSecure.delete(`/queryDelete/${id}`);
                    setQueries(queries.filter((query) => query._id !== id));
                    Swal.fire('Deleted!', 'Your query has been deleted.', 'success');
                } catch (error) {
                    console.error('Error deleting query:', error);
                    Swal.fire('Error!', 'Something went wrong while deleting the query.', 'error');
                }
            }
        });
    };
    ...
    return (
        <div className="min-h-[calc(100vh-120px)] py-4">
            <div className="relative bg-cover bg-center min-h-[calc(100vh-320px)] flex items-center justify-center"
                style={{
                    backgroundImage:
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-16">
                    <h2 className="text-3xl font-bold text-gray-700 mb-4">No Queries Found</h2>
                    <p className="text-lg text-gray-600 mb-6">You don't have any queries yet. Start by adding a new query.</p>
                    <Link to="/addQueries">
                ...
                <div>
                    <div className='flex gap-3 p-8 mx-auto w-fit'>
                        <button
                            ...
                            className={`px-4 py-2 rounded-md ${gridCols === "grid-cols-1" ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-700"}`}
                        >
                            1 Column
                        </button>
                        <button
                            onClick={() => setGridCols("grid-cols-2")}
                            className={`px-4 py-2 rounded-md ${gridCols === "grid-cols-2" ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-700"}`}
                        >
                            2 Columns
                        </button>
                        <button
                            onClick={() => setGridCols("grid-cols-3")}
                            className={`px-4 py-2 rounded-md ${gridCols === "grid-cols-3" ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-700"}`}
                        >
                            3 Columns
                        </button>
                    </div>
                    <div className={`grid ${gridCols} gap-3 lg:gap-6`}>
                        {queries.map((item) => (
                            <Zoom key={item._id}>
                                <div className="bg-white rounded-lg h-82 shadow-md p-1 border transform transition duration-300  hover:shadow-xl">
                                    <Fade>
                                        ...
                                        <p className="text-gray-600 mb-2">{item.queryTitle.slice(0,40)}...</p>
                                        <p className="text-gray-500 mb-2"><strong>Recommendations:</strong> {item.recommendationCount}</p>
                                        <p className="text-red-600 font-semibold">{item.boycott.slice(0,40)}...</p>
                                    </Fade>
